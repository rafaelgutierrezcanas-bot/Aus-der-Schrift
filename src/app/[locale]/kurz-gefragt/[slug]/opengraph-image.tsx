...
const ogQuery = `*[_type == "kurzGefragt" && (slug.current == $slug || slugEn.current == $slug) && (status == "published" || !defined(status))][0]{
...
export default async function OGImage({
  ...
  let item: { questionDe?: string; questionEn?: string } | null = null;
  try {
    item = await client.fetch(ogQuery, { slug });
  } catch {
    /* fallback to generic */
  }
  ...
  const question =
    locale === "en" && item.questionEn ? item.questionEn : (item.questionDe ?? "");

  const len = question.length;
  const titleSize = len > 100 ? 32 : len > 80 ? 36 : len > 60 ? 42 : 48;
